...
class MAML_learner(object):
    ...
    def fast_adapt(batch, learner, loss, adaptation_steps, shots, ways):
        ...
        adaptation_indices[np.arange(shots * ways) * 2] = True  #shots=1 ,ways=4, np.arange(shots * ways)=[0 1 2 3]返回一个排列 ,np.arange(shots * ways) * 2=[0 2 4 6]
                                                                #adaptation_indices=[True False True False True False True False]
    ...
    def test(self, load_path, inner_steps=10, shots=5):
        ...
        # meta_lr = 0.005  # 0.005, <0.01
        fast_lr = 0.05  # 0.01
        test_tasks = self.build_tasks('test', test_ways, shots, 200, None) #原本是1000
        maml = l2l.algorithms.MAML(self.model, lr=fast_lr)
        loss = torch.nn.CrossEntropyLoss(reduction='mean')
...
if __name__ == "__main__":
    ...
    path = r"D:\元学习\model_save\meta_learning\MAML\5shot\MAML_T1"
    Net.train(save_path=path, shots=5)
    load_path = r"D:\元学习\model_save\meta_learning\MAML\5shot\MAML_T1_ep100"
    Net.test(load_path, inner_steps=10, shots=5)
    # os.remove(r"D:\元学习\model_save\meta_learning\MAML\5shot\MAML_T1_ep100")
    #
    # if input('Train? y/n\n').lower() == 'y':
