...
class ProtoNet_learner(object):
    def __init__(self, ways):
        ...
        # self.model = Net4CNN(hidden_size=64, layers=4, channels=1, se=True).to(device)

        self.ways = ways

    @staticmethod
    def build_tasks(mode='train', ways=5, shots=5, num_tasks=100, filter_labels=None):
    ...
    @staticmethod
    # # Not introducing elasticity factor
    ...
    def euclidean_scaled_metric(query_x, proto_x, scale_factor):
        """
        :param query_x: (n, d), N-example, P-dimension for each example; zq
        :param proto_x: (nc, d), M-Way, P-dimension for each example, but 1 example for each Way; z_proto
        :param scale_factor: #Scaling factor used to control distance calculation results
        :return: [n, nc]
        """
        query_x = query_x.unsqueeze(dim=1)  # [n, d]==>[n, 1, d]
        proto_x = proto_x.unsqueeze(dim=0)  # [nc, d]==>[1, nc, d]

        distances = torch.pow(query_x - proto_x, 2).sum(dim=-1)  # Calculate Euclidean distance
        distances_scaled = distances / scale_factor  # introducing elasticity factor
        logits = -distances_scaled  # Take negative and return
        return logits
    ...
    def fast_adapt(self, batch, learner, loss_fun, query_num, shots, ways):
        ...
        # logits = self.euclidean_metric(query, support) #Not introducing elasticity factor
        logits = self.euclidean_scaled_metric(query, support, 0.01) #introducing elasticity factor
    ...
    def train(self, save_path, shots):
        ...
        for ep in range(Epochs):
            ...
            if (ep + 1) >= 100 and (ep + 1) % 2 == 0: #When changing the epoch, it needs to be modified here
    ...
    def test(self, load_path, shots):
        ...
        test_tasks = self.build_tasks('test', test_ways, shots, 200, None)
        loss_fun = torch.nn.CrossEntropyLoss()
...
if __name__ == "__main__":
    ...
    seed_torch(2023)
    Net = ProtoNet_learner(ways=5)
    path = r"D:\元学习\model_save\meta_learning\ProtoNet\5shot\ProtoNet_T1"
    Net.train(save_path=path, shots=5)
    load_path = r"D:\元学习\model_save\meta_learning\ProtoNet\5shot\ProtoNet_T1_ep100"
    Net.test(load_path, shots=5)
    # os.remove(r"D:\元学习\model_save\meta_learning\ProtoNet\5shot\ProtoNet_T1_ep100")
