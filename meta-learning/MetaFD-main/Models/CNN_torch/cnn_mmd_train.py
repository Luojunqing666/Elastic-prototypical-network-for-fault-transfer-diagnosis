...
class CNN_MMD_learner(object):
    ...
    def train_cnn(self, save_path):
        ...
        train_dataset = CNN_DataGenerator_torch(mode='train', ways=4, shot=self.shots)
        test_dataset = CNN_DataGenerator_torch(mode='test', ways=4, shot=self.shots)
        valid_dataset = CNN_DataGenerator_torch(mode='validation', ways=4, shot=self.shots)
    ...
    def test_cnn(self, load_path):
        ...
        test_dataset = CNN_DataGenerator_torch(mode='test', ways=4, shot=self.shots)  ##加了参数ways=4
...
if __name__ == "__main__":
    ...
    seed_torch(2021)
    Net = CNN_MMD_learner(ways=4, shots=5)

    if input('Train? y/n\n').lower() == 'y':
        path = r"D:\元学习\model_save\meta_learning\CNN\cnn_mmd\5shot\cnn_mmd_C30"
        Net.train_cnn(save_path=path)
    if input('Test? y/n\n').lower() == 'y':
        load_path = r"D:\元学习\model_save\meta_learning\CNN\cnn_mmd\5shot\cnn_mmd_C30_ep100"  #原本50
